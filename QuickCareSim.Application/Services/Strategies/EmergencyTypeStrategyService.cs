...
{
    public class EmergencyTypeStrategyService : IAttentionStrategyService
    {
            ...
            CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                ...
                lock (_lock)
                {
                    ...
                    var sorted = snapshot
                        .OrderByDescending(x => x.p.Urgency)
                        .ToList();
                    ...
                    if (sorted.Any())
                    {
                        ...
                        while (queue.TryDequeue(out var qid))
                        {
                            if (qid != targetId)
                                tempQueue.Enqueue(qid);
                            else
                                break;
                        }
